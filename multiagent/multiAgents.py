...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        for i in range(len(newFood.asList())):
            food_distance.append(manhattanDistance(newFood.asList()[i], newPos))

        for j in range(len(ghost_state)):
            ghost_distance.append(manhattanDistance(ghost_state[j], newPos))

        return 1 / min(food_distance) - 1 / min(ghost_distance)
...
def betterEvaluationFunction(currentGameState):
    ...
    min_food = 0 if not food_distance else 1 / min(food_distance)
    min_ghost = 0 if not food_distance else 1 / min(ghost_distance)
